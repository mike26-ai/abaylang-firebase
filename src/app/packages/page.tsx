...
import { ArrowLeft, Check, Users, Package, Calendar, Star, Info } from "lucide-react"
...
export default function PackagesPage() {
  ...
  const privateGroup = Object.values(products).find(p => p.type === 'private-group');
                    ...
                    <CardContent className="p-8 grid md:grid-cols-2 gap-8 items-center">
                        <div>
                            <Badge className="bg-primary text-primary-foreground mb-3">Unique Offering</Badge>
                             <h2 className="text-2xl font-bold text-foreground mb-2">{privateGroup.label}</h2>
                             <p className="text-muted-foreground mb-4">{privateGroup.description}</p>
                             <ul className="space-y-2 mb-6">
                                {privateGroup.features.map((feature: string, index: number) => (
                            ...
                            <Button asChild size="lg">
                                <Link href="/bookings/private-group">Create Your Private Group</Link>
                            </Button>
                        </div>
              ...
              >
                ...
                <CardContent>
                  ...
                  <Button asChild className="w-full">
                     <Link href={`/bookings?lessonType=${(pkg as any).id || pkg.label.toLowerCase().replace(/\s/g, '-')}`}>Get Package</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-2 gap-8 max-w-4xl mx-auto">
            {groupSessions.map((session) => (
              <Card key={session.label} className="border-border shadow-lg">
                <CardHeader className="text-center">
                  <div className="flex items-center justify-center gap-2 mb-2">
                ...
                <CardContent>
                  ...
                  <Button asChild className="w-full">
                    <Link href={`/bookings/group?lessonType=${(session as any).id || session.label.toLowerCase().replace(/\s/g, '-')}`}>View Schedule</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}
        ...
        <div className="mb-16">
          ...
          <div className="grid md:grid-cols-3 gap-8">
            {individualLessons.map((lesson) => (
              <Card key={lesson.label} className="border-border shadow-lg relative">
                ...
                <CardContent>
                  ...
                  <Button asChild className="w-full">
                    <Link href={`/bookings?lessonType=${(lesson as any).id || lesson.label.toLowerCase().replace(/\s/g, '-')}`}>Book Now</Link>
                  </Button>
                </CardContent>
              </Card>
            ))}

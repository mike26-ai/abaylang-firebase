...
export default function StudentDashboardPage() {
  ...
  const fetchBookingsAndProfile = async (currentUser: any) => {
      ...
      try {
        ...
        const q = query(bookingsCol, where("userId", "==", currentUser.uid), orderBy("createdAt", "desc"));
  ...
  const upcomingLessonsCount = useMemo(() => bookings.filter(b => !['completed', 'cancelled', 'cancelled-by-admin', 'refunded', 'credit-issued'].includes(b.status) && b.date !== 'N/A_PACKAGE').length, [bookings]);
          ...
          <>
            ...
            <Tabs defaultValue="profile" className="space-y-6"> 
              <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                <TabsList className="bg-card border w-full sm:w-auto grid grid-cols-2">
                      ...
                      <CardContent className="p-4 md:p-6">
                        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                          ...
                          <div className="flex items-center gap-2 md:gap-3 self-start md:self-center mt-2 md:mt-0 w-full md:w-auto justify-end">
                             <Badge variant={booking.status === "completed" ? "default" : booking.status.includes("cancel") ? "destructive" : "secondary"}>

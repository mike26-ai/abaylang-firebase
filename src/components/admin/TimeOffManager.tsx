...
import { Trash2, PlusCircle, CalendarOff, Loader2 } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
export function TimeOffManager() {
  ...
  const fetchTimeOff = async () => {
    setIsLoading(true);
    try {
      const blocks = await getTimeOff();
      // Firestore Timestamps need to be converted for use in the client
      const clientBlocks = blocks.map(block => ({
        ...block,
  ...
  const onSubmit: SubmitHandler<TimeOffFormValues> = async (data) => {
    ...
    try {
      // The server action will handle combining date and time into Firestore Timestamps

...
export function FooterLogo({ className }: LogoProps) {
  return (
    <Link href="/" className={cn("flex items-center group", className)}>
      {/* This is the footer version of the obelisk logo, adapted for a dark background */}
      <svg
        viewBox="0 0 100 130"
        className={cn("h-14 w-auto", className)}
        aria-label="Abylang Logo"
      >
        {/* Obelisk Shape with Cutouts - Color is now light (background) */}
        <path
          fillRule="evenodd"
          clipRule="evenodd"
        ...
        {/* The word "ABYLANG" below the obelisk - Color is brighter (primary) */}
        <text
          x="50"
          y="128"

...
export default function BookLessonPage() {
  ...
  const displayTimeSlots = useMemo(() => {
    ...
    for (const startTimeString of baseStartTimes) {
        ...
        if (isNaN(potentialStartTime.getTime())) continue; // Skip invalid times
        
        const potentialEndTime = addMinutes(potentialStartTime, userDurationMinutes);
        
        // Check for overlap with existing bookings
        const isBooked = dailyBookedSlots.some(b => {
            if (!b.startTime || !b.endTime) return false;
            const bookingStart = (b.startTime as any).toDate ? (b.startTime as any).toDate() : new Date(b.startTime);
            const bookingEnd = (b.endTime as any).toDate ? (b.endTime as any).toDate() : new Date(b.endTime);
            return potentialStartTime < bookingEnd && potentialEndTime > bookingStart;
        });

        // Check for overlap with time-off blocks
        const isBlocked = dailyTimeOff.some(t => {
  ...
  const handleBooking = async () => {
    ...
    try {
      ...
      if (isFreeTrial) {
        ...
        // The success_url is handled by Paddle's dashboard now, but we can pass it as a fallback
        const successUrl = `${window.location.origin}/bookings/success`;
        
        // This 'passthrough' data is critical for linking the Paddle transaction to our bookingId
        const passthroughData = { booking_id: bookingId };
        
        const checkoutUrl = new URL(paddleLink);
        checkoutUrl.searchParams.set('passthrough', JSON.stringify(passthroughData));
        checkoutUrl.searchParams.set('success_url', successUrl); // Recommended to set in Paddle dashboard
        checkoutUrl.searchParams.set('email', user.email || ''); // Pre-fill user email
